...
Evitar la pérdida de consultas en DM/WhatsApp: todo queda registrado.
Permitir un flujo “sistema comercial”:
...
Flujo funcional
El usuario llega a la landing (idealmente con UTMs desde redes/ads/QR).
Completa el formulario (B2B/B2C).
LeadForm llama al submitLead(raw) del controller.
Controller:
normaliza y valida (Model),
agrega UTMs + metadata (page_url, user_agent, timestamp),
envía el payload a n8n (Service).
UI muestra estado OK/ERROR y ofrece botón WhatsApp con mensaje prearmado.
...
VITE_N8N_WEBHOOK_URL=https://tudominio.com/webhook/lead-intake VITE_WHATSAPP_NUMBER=5492910000000 VITE_CATALOG_URL=https://...
